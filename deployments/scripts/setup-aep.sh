...
# env.sh already resolved ENABLE_AGENT_MANAGER (explicit env var > .env > 1);
# persist that resolved value so a one-run `ENABLE_AGENT_MANAGER=0` override
# sticks for later runs instead of being reverted to the saved .env value.
ENABLE_AGENT_MANAGER_VAL="${ENABLE_AGENT_MANAGER:-$(existing_val ENABLE_AGENT_MANAGER)}"
[ -z "$ENABLE_AGENT_MANAGER_VAL" ] && ENABLE_AGENT_MANAGER_VAL=1
...
# Local setup toggle — set to 0 for an AEP-only stack without Agent Manager.
ENABLE_AGENT_MANAGER=${ENABLE_AGENT_MANAGER_VAL}

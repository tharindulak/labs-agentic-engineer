...
// with every other component sharing the same dependency, keyed on the same
// dedupe identity the build dependency drawer groups on: same `kind` AND
// identity (`resourceType`+`name` for platform-resource, `name` alone
// otherwise).

...
import { AGENT_CHAT_PANEL_WIDTH, AgentChatPanel } from "../features/agent-chat/components/AgentChatPanel";
  ...
  return (
    <AppShell initialCollapsed={false} collapseOnSelectOnMobile>
      ...
      <AppShell.Main>
        ...
        >
          {/* A page that throws while rendering is contained here: the shell
          ...
          {projectName && (
            ...
            >
              {/* The panel sits beside the Outlet, outside the page boundary, so
                  it has its own: a throw mid-turn leaves the page usable.
                  Closing the panel unmounts it, so reopening is also a reset. */}
              <ErrorBoundary
                label="The chat panel"
                resetKey={projectName}
                fill
                // The panel sizes itself; a fallback standing in for it must
                // hold the same slot or the Collapse shrinks to nothing.
                fallbackSx={{ width: AGENT_CHAT_PANEL_WIDTH, height: "100%", borderLeft: 1, borderColor: "divider" }}
              >
                <AgentChatPanel

...
### Adoption
Making an issue workable: it joins a version's milestone — the deployed one, or
the spec build in flight when nothing is deployed yet — carries the **arming
label**, and a run is started over that milestone, or the run already parked on
it is woken.

Two routes, one meaning. An issue that already exists is adopted by a human
arming it with `aep` (in GitHub, or from the console), and the platform adds only
the milestone: arming IS the human's act, so nothing platform-side writes that
...
`deploy-budget`, `fix-chain-budget`, `conflict-budget`, `no-progress`,
`cycle-ceiling`, `validation-failed` — so a reason is an explanation rather than
a label. A run that settles for anything

# ADR-0032 — A merged fix is not a resolved incident

The SRE/RCA handoff files a GitHub issue, `AdoptOnCreate` hands it to a coding
agent (ADR-0031), the agent opens a pull request carrying `Resolves #N`, and the
...
with an `adoptionError` — a ledger entry, the same refusal shape ADR-0031 chose,
...
the answer rather than restated by the model — the rule ADR-0031 established for

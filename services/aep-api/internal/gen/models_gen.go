...
// CreateRcaAgentReportRequest Write-side request for a new RCA-agent alert report.
...
type CreateRcaAgentReportRequest struct {
	// Report The SRE agent's native report document, opaque to this contract. Deliberately unmodelled — mirroring a document owned by another repository here would recreate, in schema form, exactly the coupling that sending it whole removes. Its recommended actions are also what the escalation decision reads, as fields rather than as patterns over rendered prose.

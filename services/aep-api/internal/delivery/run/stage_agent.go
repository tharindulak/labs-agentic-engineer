...
func (l *loop) dispatchUntilLanded(ctx workflow.Context, kind string, anchorIssue int, cycleID string) (bool, cycleResult, error) {
	...
	for l.st.CycleAttempt < delivery.RunMaxRedispatchPerCycle {
		...
		if derr != nil {
			...
			// Both of these settle the run under their own reason, which is the
			// whole answer already — they need no note to carry the detail.
	...
	if !launched {
		// Logged on the way out because the reason is the whole answer for
		// whoever reads this run, and the terminal reason alone is only a
		// category — the cycle row carries the detail, and this puts it in the
		// operator's log too.
		workflow.GetLogger(ctx).Error("run: no agent could be started — settling",
			...
			"attempts", l.st.CycleAttempt, "reason", lastLaunchErr)

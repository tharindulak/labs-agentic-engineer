...
const (
	...
	testPRNumber = 42
	// testJobRef is what a launch answers with when it succeeds.
	testJobRef = "job-1"
...
type harness struct {
	...
	closed  int
	// jobRef is what a SUCCEEDING launch answers with. A field rather than a
	// constant because the single dispatch expectation is registered before any
	// test has pinned it (dispatchIs).
	jobRef string
...
func newHarness(t *testing.T) *harness {
	...
	h := &harness{env: ts.NewTestWorkflowEnvironment(), set: map[string]bool{}, jobRef: testJobRef, promoted: map[string]string{}}
	var acts *Activities
	h.acts = acts
	...
	// per-test fact, and the static form is fixed at registration — which, being
	// unlimited, no later expectation can override. So this is the ONE dispatch
...
// dispatchIs pins what launching the cycle's agent answers. Every dispatch is
// recorded either way, so the budget assertions hold for a failing dispatch too.
...
// build is green, and the project has no acceptance oracle. Dispatch is absent
// here — its single expectation is registered in newHarness with the launch
// answer already defaulted.
...
func TestAgentQuotaBlocked_SettlesBlockedWithoutSpendingTheBudget(t *testing.T) {
	...
	require.Empty(t, h.dispatchFailures,
		"a quota refusal settles under its own reason — there is no launch fault to note on the cycle")

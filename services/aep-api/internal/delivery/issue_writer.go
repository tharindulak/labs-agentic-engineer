...
func (w *IssueWriter) Mint(ctx context.Context, orgID, projectID string, spec IssueSpec) (number int, deduped bool, err error) {
	res, err := w.MintResult(ctx, orgID, projectID, spec)
	if err != nil || res == nil {
		return 0, false, err
	}
	return res.Number, res.Deduped, nil
}

// MintResult mints exactly as Mint does and answers with the host's WHOLE
// result — the number, the browser URL and the node id.
//
// It exists for the one caller that has to relay that answer rather than act on
// it: create-issue is a public endpoint, and an external handoff filing an
// incident through it cannot look the URL up for itself. Everything inside the
// platform wants the number alone, so Mint stays the ordinary door.
//
// A nil writer, or one with no host wired, answers (nil, nil) — the same
// degrade-to-nothing Mint has, for the same reason.
func (w *IssueWriter) MintResult(ctx context.Context, orgID, projectID string, spec IssueSpec) (*sourcecontrol.IssueResult, error) {
	if w == nil || w.ops == nil {
		return nil, nil
	}

...
- **Adoption makes an issue agent work — milestone membership alone does not.** The working set is the
  ...
  adopted issue without the arming label is invisible to the dispatch predicate: the run starts, finds
  nothing, and parks forever. The two routes reach that state from opposite directions. `AdoptIssue` is
  entered BY the arming — a human's `aep` webhook — so the issue is already armed and the platform adds
  only the milestone; writing `aep` itself would make "who adopted this" unanswerable. `AdoptOnCreate` is
  asked for by the caller FILING the issue, so `aep` and the milestone ride the one create call and there
  is no window in which the issue exists but is not yet workable. Neither route stamps a KIND: an armed
  issue carrying none reads as a `bug` to every working-set predicate, which is what an unclassified
  hand-over means.
...
- **A parked run is woken by whichever side wrote the work.** `wakeIfWorkable` is the one definition of
  ...
  rather than signalling twice; a create the platform made comes back as a SUPPRESSED echo, so
  `AdoptOnCreate` wakes the run itself. The choice is spelled at the call site (`wakePolicy`), because
  either mistake is silent — a missing wake leaves a run asleep on work in front of it.
